    ...
    try {
        ...
        if (intent_mandate) {
            try {
                ...
                console.error('Intent mandate parse error:', e);
                return res.status(400).json(
        ...
        // Get synced products - use AP2 platform records
        const syncedProducts = db.getSyncedProducts(merchant_id, 'ap2') || [];

        // Parse product data
        let products = syncedProducts
            .map(sp => safeParse(sp.product_data))
            .filter(Boolean);

        console.log(`Found ${products.length} AP2 products for merchant ${merchant_id}`);
        ...
        if (parsedMandate && parsedMandate.constraints) {
            ...
            if (constraints.price_limit) {
                products = products.filter(p => {
                    const priceValue = parseFloat(p.price?.value || p.price || 0);
                    const priceCents = Math.round(priceValue * 100);
                    return priceCents <= constraints.price_limit.amount;
                });
            }
        ...
        console.error('Error in AP2 /products:', error);
        res.status(500).json(
...
router.post('/carts', async (req, res) => {
    try {
        ...
        console.error('Error in AP2 /carts:', error);
        res.status(500).json(
            AP2Adapter.createErrorResponse('server_error', error.message)
        );
    }
...
router.get('/carts/:id', (req, res) => {
    try {
        ...
        console.error('Error in AP2 /carts/:id:', error);
        res.status(500).json(
            AP2Adapter.createErrorResponse('server_error', error.message)
        );
    }
...
router.put('/carts/:id', async (req, res) => {
    try {
        ...
        console.error('Error in AP2 /carts/:id update:', error);
        res.status(500).json(
            AP2Adapter.createErrorResponse('server_error', error.message)
        );
    }
...
router.post('/payments/authorize', async (req, res) => {
    try {
        ...
        let verification;
        if (intentMandate) {
            const parsedIntent = safeParse(intentMandate.mandate_data);
...
router.get('/transactions/:id', (req, res) => {
    try {
        ...
        res.json({
            ...
            transaction: {
                ...transaction,
                audit_trail: safeParse(transaction.audit_trail)
            }
        });
    } catch (error) {
        console.error('Error in AP2 /transactions/:id:', error);
        res.status(500).json(
            AP2Adapter.createErrorResponse('server_error', error.message)
        );
    }
