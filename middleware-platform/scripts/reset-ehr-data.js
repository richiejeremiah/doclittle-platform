/**
 * Reset EHR Data - Delete all and recreate only medical coding test data
 */
...
console.log('🗑️  Resetting EHR Data...\n');
console.log('='.repeat(60));

// Step 1: Delete all existing EHR data
console.log('📋 Step 1: Deleting all existing EHR data...');

const deletedObservations = db.prepare('DELETE FROM ehr_observations').run();
console.log(`   ✅ Deleted ${deletedObservations.changes} observations`);

const deletedProcedures = db.prepare('DELETE FROM ehr_procedures').run();
console.log(`   ✅ Deleted ${deletedProcedures.changes} procedures`);

const deletedConditions = db.prepare('DELETE FROM ehr_conditions').run();
console.log(`   ✅ Deleted ${deletedConditions.changes} conditions`);

const deletedEncounters = db.prepare('DELETE FROM ehr_encounters').run();
console.log(`   ✅ Deleted ${deletedEncounters.changes} encounters`);

console.log('\n✅ All EHR data deleted\n');

// Step 2: Recreate only medical coding test data
console.log('📋 Step 2: Creating medical coding test data...\n');
...
fhirPatients.forEach((patient, patientIdx) => {
  ...
  // Assign 1 scenario per patient
  const scenario = testScenarios[patientIdx % testScenarios.length];
  ...
  const encounterDate = new Date(Date.now() - (patientIdx * 7 * 24 * 60 * 60 * 1000));
...
console.log('='.repeat(60));
console.log('✅ EHR Data Reset Complete!\n');
...
console.log('🔄 Now refresh your browser to see the test data in the Clients tab!');
